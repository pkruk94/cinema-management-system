...
import com.app.validation.movie.CreateMovieValidator;
import com.app.validation.movie.UpdateMovieValidator;
...
public class MovieService {
    ...
    public Long addMovie(CreateMovieDto createMovieDto) {
        ...
        if (movieRepository.findMovieByTitleAndReleaseDate(createMovieDto.getTitle(), createMovieDto.getReleaseDate()).isPresent()) {
            throw new MovieServiceException("Such com.app.movie already exists in database");
        }
    ...
    public Long updateMovie(UpdateMovieDto updateMovieDto) {
        ...
        var movie = movieRepository
                ...
                .orElseThrow(() -> new MovieServiceException("Such com.app.movie does not exist in database"));
    ...
    public boolean addMovieToFavourites(UpdateFavouriteMovie updateFavouriteMovie) {
        if (Objects.isNull(updateFavouriteMovie.getMovieId()) || Objects.isNull(updateFavouriteMovie.getUserId())) {
            throw new MovieServiceException("Neither com.app.movie ID nor com.app.user ID cannot be null");
        }

        User user = userRepository.findById(updateFavouriteMovie.getUserId()).orElseThrow(() -> new MovieServiceException("User could not be found while assigning favourite com.app.movie"));
        Movie movie = movieRepository.findById(updateFavouriteMovie.getMovieId()).orElseThrow(() -> new MovieServiceException("Movie could not be found while assigning to com.app.user"));
    ...
    public boolean removeMovieFromFavourites(RemoveFavouriteMovie removeFavouriteMovie) {
        ...
        User user = userRepository.findById(removeFavouriteMovie.getUserId()).orElseThrow(() -> new MovieServiceException("User could not be found while assigning favourite com.app.movie"));
        Movie movie = movieRepository.findById(removeFavouriteMovie.getMovieId()).orElseThrow(() -> new MovieServiceException("Movie could not be found while assigning to com.app.user"));
